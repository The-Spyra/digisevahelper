  ...
  async function getImageAspectRatio(metadata: sharp.Metadata): Promise<string> {
    try {
      ...
      if (metadata.width && metadata.height) {
        ...
        throw new Error('Unable to determine aspect ratio: width or height missing.');
      }
    } catch (error) {
...
export async function mergeImagesWithWatermark(
  ...
  try {
    ...
    console.log(ratio,Math.floor((image1Metadata.width!/image2Metadata.width!)*image2Metadata.height!))


    // Resize the second image to match the width of the first image
    const resizedImage2 = await image2
      .resize({ width: image1Metadata.width,fit:"fill",height:Math.floor((image1Metadata.width!/image2Metadata.width!)*image2Metadata.height!)})
      .toBuffer();
    ...
    await sharp({
      ...
      .composite([
        ...
        { input: phonebuff, top: Math.floor((image1Metadata.width || 50)/30), left: 20 },
